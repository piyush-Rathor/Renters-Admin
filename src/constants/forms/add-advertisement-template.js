...
const addAdvertisementTemplate = {
  _type: 'object',
  ...
  subSubcategory: {
    ...
    validator: Yup.string()
      .label('sub-subcategory')
      .when('target', (t, schema) => (t === 'Sub-Subcategory' ? schema.required('Required') : schema)),
...
export const addAdvertisementDisplayLocation = {
  _type: 'object',
  ...
  category: {
    ...
    validator: Yup.string()
      .when('displayLocation', (t, schema) => (t === 'Category' ? schema : schema.notRequired())),

    _hide: (currentPath, { backTracePath, getValueAtPath }) => {
      return !['Category', 'Subcategory'].includes(
        getValueAtPath(['displayLocation']),
      )
    },
    breakpoints: { xs: 8 },
  },
  subcategory: {
    ...
    validator: Yup.string()
      .required('Required')
      .when('displayLocation', (t, schema) => (t === 'Subcategory' ? schema : schema.notRequired())),

    _hide: (currentPath, { backTracePath, getValueAtPath }) => {
      return !['Collection', 'Subcategory', 'Sub-Subcategory'].includes(getValueAtPath(['displayLocation']))
    },
    breakpoints: { xs: 6 },
  },
}

export default addAdvertisementTemplate
